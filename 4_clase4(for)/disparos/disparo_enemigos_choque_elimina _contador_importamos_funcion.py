...
import sys
import random
from misfunciones import dibuja_mensaje
# Inicializar Pygame
pygame.init()
...
#Fuentes: una vez que el alumno haya visto cómo se escribe la fuente
#Sería interesante que no duplciara código
...
while True:
    ...
    #Colisión entre enemigos y el jugador
    for e in enemigos:
            if e.colliderect(jugador):
                vidas -= 1
                #Para evitar que el mismo rectángulo choque varias veces lo desviamos
                #Probar a comentar la siguiente línea
                e.x += 100
    ...
    if not enemigos:
        screen.blit(*dibuja_mensaje("Has ganado", None, 60, (255,255,0),(200,200)))
    # Si no quedan vidas
    if vidas < 0:
        screen.blit(*dibuja_mensaje("Has sido eliminado", None, 60, (255,255,0),(200,200)) )
    # Con este control de estados sigue permitiéndose, en el caso de eliminar al jugador no se deben mostrar los enemigos
    # ni la posibilidad de disparos.        
    screen.blit(*dibuja_mensaje(f"Enemigos:{len(enemigos)} vidas{vidas}", None, 30, (255,255,0),(10,10)))
    pygame.display.flip()
    clock.tick(60)
